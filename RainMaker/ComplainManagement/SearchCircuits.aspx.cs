...
namespace RainMaker.ComplainManagement
{
    public partial class SearchCircuits : System.Web.UI.Page
    {
        ...
        protected void gvActiveCircuits_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                string Check = e.CommandName;
                int index = Convert.ToInt32(e.CommandArgument);
                GridViewRow item = gvActiveCircuits.Rows[index];
                if (cmbComplainAt.SelectedValue == "0" || cmbTicketType.SelectedValue == "N/A")
                {
                    //Interaction.MsgBox("Please Select Above Inforamtion First", MsgBoxStyle.Information, "BSS Adminstrator");
                    return;
                }
                Session["Complain_SignupID"] = item.Cells[3].Text;
                Session["Complain_CMSID"] = item.Cells[4].Text;
                Session["Complain_InfraID"] = item.Cells[20].Text;
                Session["Complain_InfraCode"] = item.Cells[23].Text;
                ...
                if (Check == "ComplainHistory")
                {
                    Session["Complain_MultiFlag"] = "ViewSignupTransaction";
                    string click = "ViewSignupTransaction";
                    Response.Redirect("~\\ComplainManagement\\ViewHistory.aspx?SignupID=" + Convert.ToInt32(item.Cells[3].Text) + "&CMSID=" + Convert.ToInt32(Session["Complain_CMSID"]) + "&click=" + click);
